...
func TestSensorsHistory(t *testing.T) {
	...
	endDate := "2010-01-02T15:07:05.999Z"
	t.Run("GET_sensors/sensor_id/history", func(t *testing.T) {
		t.Run("valid_request_200", func(t *testing.T) {
			w := httptest.NewRecorder()
		...
		t.Run("sensor_not_found_404", func(t *testing.T) {
			w := httptest.NewRecorder()
		...
		t.Run("unexpected_id_format_422", func(t *testing.T) {
			w := httptest.NewRecorder()

			req, _ := http.NewRequest(http.MethodGet, "/sensors/fsajdklfsfjaslkdf/history", nil)
			req.Header.Add("Accept", "application/json")
		...
		t.Run("unexpected_body_format_400", func(t *testing.T) {
			w := httptest.NewRecorder()
			...
			q := req.URL.Query()
			q.Add("start_date", "dsanfdsaklfsaf")
			q.Add("end_date", "fdkfskafksd")
			req.URL.RawQuery = q.Encode()
